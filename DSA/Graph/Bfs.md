

TARGET DECK: DSA::Graph::Bfs

#flashcards/DSA/Graph/Bfs


Q: [1345. Jump Game IV](https://leetcode.com/problems/jump-game-iv/)
think how or which category problem can be then solve it
?
A: it has lot of options then maybe recursion or dp for bruteforce but the main issue is we can visit a index again try to see write a test case as from i, i+1 then back to i and it repeat
next comes is to use of visited array so that we cannot visited same state again. Now it is looking like a graph as it asked for min no of step so bfs is suitable for shorted path in unweighted graph== bfs in graph simple
source codestorywithmik link - https://www.youtube.com/watch?v=tr1WHelOhJU
my code link - https://leetcode.com/problems/jump-game-iv/submissions/2005977800/
<!--ID: 1779073840093-->

<!--SR:!2026-05-21,3,250-->

